...
function sanitizeCountryLabel(label){
  // Keep only the country text before the first emoji/flag, e.g. "American 🇺🇸" -> "American"
...
function buildCountryMenu(menuEl){
  menuEl.innerHTML = "";
  const items = [{label:"Random", value:"random"}, ...COUNTRIES.map(c => ({label:c, value:sanitizeCountryLabel(c)}))];
...
function resolveSelection(genderValue, countryValue){
  ...
  if (!COUNTRY_VALUES.includes(country)) country = pickRandomCountry();
